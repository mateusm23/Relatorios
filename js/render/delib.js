...
function buildTable(rows, ttl, ico, bg) {
  ...
  const trs = rows.map(r => `<tr>${cols.map(c => {
    const vv = r[c];
    const kk = normKey(c);
    if (kk === 'STATUS') return `<td>${statusDelibTag(vv)}</td>`;
    if (kk.includes('PRAZO') || kk.includes('DATA')) return `<td>${fmtDate(vv)}</td>`;
    if (kk.includes('DELTA')) {
      const nn = Number(vv) || 0;
      return `<td style="font-weight:700;color:${nn < 0 ? 'var(--vermelho)' : 'var(--verde)'}">${vv}</td>`;
    }
    return `<td style="max-width:200px;white-space:normal">${vv || '—'}</td>`;
  }).join('')}</tr>`).join('');

  return `<div class="card">
    <div class="card-hd">
      <div class="card-ico" style="background:${bg}">${ico}</div>
...
export function renderDelib() {
  ...
  document.getElementById('delibContent').innerHTML =
    buildTable(pend, 'Demandas em Aberto', '⏳', '#B45309') +
    buildTable(conc, 'Concluídas', '✅', '#217A3C');
  markDone(5);
}
